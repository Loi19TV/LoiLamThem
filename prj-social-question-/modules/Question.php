...
class Question extends Database
{
    ...
    // Phương thức xóa question
    public function deleteQuestionById($id)
    {
        try {
            $sql = parent::$connection->prepare("DELETE FROM questions WHERE id = ?");
            $sql->bind_param("i", $id); // Sửa "s" thành "i" cho kiểu integer
            ...
            echo "Thất bại, có lỗi xảy ra trong Module/User/deleteQuestionById: " . $th;
            return false;
        }
    }


    // Phương thức cập nhật question
    public function updateQuestionById($id, $content, $type, $author)
    {
        try {
            ...
            // Chuẩn bị câu lệnh SQL để cập nhật dữ liệu trong bảng questions
            $sql = parent::$connection->prepare("UPDATE questions SET content = ?, type = ?, edited_at = ?, author = ? WHERE id = ?");
            $sql->bind_param("ssssi", $content, $type, $edited_at, $author, $id);

            // Thực hiện truy vấn cập nhật
            $success = $sql->execute();

            // Kiểm tra kết quả và trả về kết quả
            if ($success) {
                return true;
            } else {
                return false;
            ...
            echo "Thất bại, có lỗi xảy ra trong Module/User/updateQuestionById: " . $th;
            return false;
        }
    }
